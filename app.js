...
function normalizeReviewFilter(input) {
  ...
  return {
    ...
    minScore: clampNumber(source.minScore, 2, 1, 6),
...
function isLikelyReview(text, filterConfig) {
  ...
  if (isShortCandidate) {
    if (GREETING_ONLY_PATTERN.test(compact)) {
      return false;
    }

    return SHORT_REVIEW_SIGNAL_PATTERN.test(normalized) || FEEDBACK_INTENT_PATTERN.test(normalized);
  }
  ...
  const hasFeedbackIntent = FEEDBACK_INTENT_PATTERN.test(normalized);
